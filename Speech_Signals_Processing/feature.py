...
def m4a_to_wav(filepath):
    ...
    command = "ffmpeg -i"+" "+filepath+" "+"-ar 16000 -vn"+" "+ filename+".wav"
...
def trans_mp3_to_wav(filepath):
    '''
    :param filepath: 输入m4a文件路径
    :return: wav
    '''
    filename = filepath.split('.')[0] + '.wav'
    print(filename)
...
if __name__ == '__main__':
    filepath = 'resources/00001.m4a'
    filename = m4a_to_wav(filepath)
    print(filename)
    wavsignal, fs = read_wav_data(filename)

    a = get_mfcc_feature(wavsignal , fs)
    b = get_fbank_feature(wavsignal , fs)
    c = get_frequency_feature(wavsignal , fs)
    print(a.shape)
    print(b.shape)
    print(c.shape)